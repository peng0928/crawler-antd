...
export default () => {
  ...
  const TestDellALL = (row, onCleanSelected) => {
    axios({
      method: 'post',
    ...
    <div>
      <ProTable<GithubIssueItem>
        ...
        tableAlertOptionRender={({ selectedRowKeys, onCleanSelected }) => {
          return (
            <Space size={16}>
              <Button danger onClick={() => TestDellALL(selectedRowKeys, onCleanSelected)}>
                全部删除
              </Button>
              <Button type="primary">全部运行</Button>
            </Space>
          );
        }}
        actionRef={actionRef}
