...
export default function OnlineEdit() {
  ...
  const handleUpload = () => {
    ...
    for (let i = 0; i < spiderfile.length; i++) {
      formData.append('files', spiderfile[i]);
    }
    setUploading(true);
  ...
  const props = {
    ...
    multiple: true,
    onChange: (info) => {
      spiderfile.append(info.file.name, info);
      console.log('set', info.fileList);
      return false;
    },
    beforeUpload: () => {
      return false;
  ...
  return (
    <div>
      ...
      >
        ...
        <Upload action="http://127.0.0.1:4640/api/upload/file" method="POST" directory>
          <Button icon={<UploadOutlined />}>上传</Button>
        </Upload>
      </Modal>
    </div>
