...
export default function OnlineEdit() {
  ...
  const trans_val = (data2) => {
    console.log(data2);
    let arr = '1';
    let new_arr = [];
    new_arr = arr.map((item, index) => ({
      title: item, //设置字段，在form.list下的form.item中指定的字段值
      xpath: item, //设置字段，在form.list下的form.item中指定的字段值
      filter: item, //设置字段，在form.list下的form.item中指定的字段值
      fieldKey: index,
      isListField: true,
      key: index,
      name: index,
  ...
  useEffect(() => {
    axios({
      ...
      .then(function (response) {
        ...
        const listing = [listing_name, listing_value, listing_filter];
        console.log(data, params);
        form.setFieldsValue({
          starturl: data.starturl,
          host: data.host,
          spider: data.spider,
          LData: trans_val(listing),
        });
      })
